...
resource "aws_wafv2_web_acl_logging_configuration" "waf_logging" {
    ...
    content {
      ...
      dynamic "filter" {
        ...
        content {
          ...
          dynamic "condition" {
            for_each = filter.action_condition != null ? filter.action_condition : []
          ...
          dynamic "condition" {
            for_each = filter.label_name_condition != null ? filter.label_name_condition : []

            content {
              label_name_condition {
                label_name = condition.label_name
              }
            }
          }
        }
  ...
  dynamic "redacted_fields" {
    for_each = var.waf_logging_configuration.redacted_fields == null ? var.waf_logging_configuration.redacted_fields : []

    content {
      dynamic "method" {
        for_each = redacted_fields.method == true ? [true] : []
      ...
      dynamic "single_header" {
        for_each = redacted_fields.single_header != null ? [true] : []

        content {
          name = redacted_fields.single_header.name
        }
      }

      dynamic "uri_path" {
        for_each = redacted_fields.uri_path == true ? [true] : []
